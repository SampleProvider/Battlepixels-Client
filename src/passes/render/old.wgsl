...
@group(0) @binding(2) var<uniform> player: vec2<f32>;
@group(0) @binding(3) var<uniform> time: f32;
@group(1) @binding(0) var<uniform> grid_size: vec2<u32>;
@group(1) @binding(1) var<uniform> chunk_size: vec4<u32>;
@group(1) @binding(2) var<storage, read> grid: array<f32>;
@group(1) @binding(3) var<storage, read> chunks: array<u32>;
@group(1) @binding(4) var<storage, read> colors: array<vec4<f32>>;
...
fn raycast(pos: vec2<f32>, d: vec2<f32>) -> f32 {
    // let norm_d = d / length(d) * exp(length(d) / 100.0);
    let norm_d = d / length(d);
    // let norm_d = d / 100.0;
    var new_pos = pos;
    var obscured = 0.0;
    let plane = max(sign(norm_d), vec2<f32>(-1e-10, -1e-10));
    let inv_d = 1 / norm_d;
    var times = 0;
    for (var i = 0; i < 100; i++) {
        let index = (u32(new_pos.x) + u32(new_pos.y) * grid_size.x) * stride;
        let id = u32(grid[index]);

        var minNext = 1.0;

        // minNext *= 1 - f32(id == 1 || id == 4);
        minNext *= 1 - f32(new_pos.x < 0 || u32(new_pos.x) >= grid_size.x || new_pos.y < 0 || u32(new_pos.y) >= grid_size.y);
        // if (id == 1 || id == 4) {
        //     return travelled;
        // }

        // let next = (plane - (new_pos - floor(new_pos))) * inv_d;
        // var minNext = min(next.x, next.y);
        // minNext = 1.0;

        new_pos += norm_d * minNext;

        obscured += exp(-f32(i) / 10) * minNext * f32(id == 1 || id == 4);
...
fn fs_main(@builtin(position) pos: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // let chunk_index = (u32(pos.x / f32(chunk_size.x)) + u32(pos.y / f32(chunk_size.y)) * chunk_size.z) * chunk_stride;
    let id = u32(grid[index]);
    var color = colors[id * 2];
    // if (u32(pos.x) >= chunks[chunk_index] && u32(pos.x) <= chunks[chunk_index + 1] && u32(pos.y) >= chunks[chunk_index + 2] && u32(pos.y) <= chunks[chunk_index + 3]) {
    ...
    //     color = mix(color, vec4<f32>(1.0, 1.0, 1.0, 1.0), 0.75);
    //     for (var y = i32(pos.y) - 1; y >= 0; y--) {
    //         if (u32(grid[(u32(pos.x) + u32(y) * grid_size.x) * stride]) == 0) {
    //             break;
    //         }
    //         color = mix(color, colors[id * 2], 0.5);
    //     }
    // }
    // let obscured = raycast(new_pos, vec2<f32>(0.5, -1));
    // new_pos = floor(new_pos) + vec2<f32>(0.5, 0.5);
    // // let obscured = raycast(new_pos, player - new_pos);
    // let obscured = raycast(new_pos, vec2<f32>(0.5, -1));
    // if (id != 1 && id != 4) {
    // // color = mix(color, vec4<f32>(1.0, 1.0, 0.75, 1.0), 0.5);
    // color = mix(color, vec4<f32>(0.0, 0.0, 0.0, 1.0), 1 - exp(-obscured / 1000.0 * length(player - new_pos)));
    // }
    return color;
    // return vec4<f32>(new_pos.x / 100, 0.0, 0.0, 1.0);
}